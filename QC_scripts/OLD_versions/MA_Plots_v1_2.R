...
MAPlots_check <- function(X,...){
  number_states <- max(as.integer(as.factor(colnames(X))))
  number_plots <- choose(number_states,2)
  return_value <- 2

  if(number_plots >= 1000){
    beepr::beep(sound = 10)
    user_input <- readline(prompt = paste("Are you sure you want to plot ",number_plots," MA-plots? [yes/no]"))
    if (user_input == "yes"){
      return_value <- 1
    }else return_value <- 0
  }else return_value <- 1
...
MAPlots <- function(X, log = TRUE, alpha = FALSE, suffix="nonorm",
                    labels = 1:ncol(X), labels2 = colnames(X),
...
### version 1.2:
# if more than 1000 plots will be plotted, the user is asked and the number of
#   plots is shown
